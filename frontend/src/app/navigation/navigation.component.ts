...
@Component({
  ...
  template: `
<nav class="navbar" *ngIf="!isAuthPage()">
...
<li class="nav-item dropdown" (mouseleave)="closeDropdown('user')">
  ...
  >
    <span class="nav-icon">👤</span>
    <span class="nav-text">{{ currentUser?.fullName || currentUser?.email || 'Account' }}</span>
    <svg class="dropdown-arrow" width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="2">
      <path d="M3 4.5L6 7.5L9 4.5"/>
    </svg>
  </a>
  <ul class="dropdown-menu" [class.show]="activeDropdown === 'user'">
    ...
    <li>
      <a routerLink="/profile" routerLinkActive="active" (click)="closeAll()" class="dropdown-item">
        <span class="dropdown-icon">🪪</span>
        <div>
          <div class="dropdown-title">{{ currentUser?.fullName || 'My Account' }}</div>
          <div class="dropdown-desc">{{ currentUser?.email }}</div>
        </div>
      </a>
    </li>
    ...
    <li>
      <a (click)="logout()" class="dropdown-item" style="cursor: pointer;">
        <span class="dropdown-icon">🚪</span>
        <div>
          <div class="dropdown-title">Logout</div>
          <div class="dropdown-desc">Sign out of your account</div>
        </div>
      </a>
    </li>
...
export class NavigationComponent implements OnInit {
  ...
  currentUser: User | null = null; // ✅ holds logged-in user info
