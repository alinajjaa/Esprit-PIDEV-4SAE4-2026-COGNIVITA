...
export class UsersListComponent implements OnChanges {
  ...
  applyFilters(): void {
    ...
    if (this.statusFilter === 'ACTIVE')  result = result.filter(u => !u.blocked);
    if (this.statusFilter === 'BLOCKED') result = result.filter(u => u.blocked);
  ...
  get paginatedUsers(): User[]   { const s = (this.currentPage - 1) * this.pageSize; return this.filteredUsers.slice(s, s + this.pageSize); }
  get pageNumbers():    number[] { return Array.from({ length: this.totalPages }, (_, i) => i + 1); }

  goToPage(page: number): void {
    if (page >= 1 && page <= this.totalPages) { this.currentPage = page; this.mark(); }
  }
  ...
  showToast(message: string, type: 'success' | 'error' | 'warning'): void {
    ...
    this.mark();
    this.toastTimeout = setTimeout(() => { this.toast = null; this.mark(); }, 3500);
  }
  ...
  async exportPdf(): Promise<void> {
    ...
    const stats = [
      ...
      { label: 'Active',      value: this.totalUsers - this.totalBlocked, color: [0,255,136]  },
      { label: 'Blocked',     value: this.totalBlocked,                   color: [255,50,80]  },
    ];

    const boxW = 44, boxH = 16, startX = 14, startY = 34, gap = 50;
    stats.forEach((s, i) => {
    ...
    autoTable(doc, {
      ...
      body: tableData,
      theme: 'plain',
      styles: { font: 'helvetica', fontSize: 9, textColor: [176,224,255], cellPadding: { top: 4, bottom: 4, left: 6, right: 6 }, lineColor: [0,50,70], lineWidth: 0.3, fillColor: [10,10,26] },
      headStyles: { fillColor: [0,30,50], textColor: [0,255,255], fontStyle: 'bold', fontSize: 9, lineColor: [0,255,255], lineWidth: 0.4 },
      alternateRowStyles: { fillColor: [0,15,25] },
      didParseCell: (data: any) => {
        if (data.column.index === 3 && data.section === 'body') {
          data.cell.styles.textColor = data.cell.raw === 'Blocked' ? [255,50,80] : [0,255,136];
        }
        if (data.column.index === 2 && data.section === 'body') {
          data.cell.styles.textColor = data.cell.raw === 'ADMIN' ? [255,215,0] : [0,200,255];
        }
      }
    });
