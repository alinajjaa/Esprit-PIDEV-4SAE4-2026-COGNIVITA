...
export class RendezvousList implements OnInit {
  ...
  private rdvApiUrl = 'http://localhost:8089/rdv/api/rendezvous';
  private medecinApiUrl = 'http://localhost:8089/rdv/api/medecins';
  ...
  ngOnInit(): void {
    ...
    // Load doctors options (non-blocking)
  ...
  loadRendezVousForPatient(): void {
    this.http.get<any[]>(`${this.rdvApiUrl}/patient/${this.patientId}`)
      .pipe(finalize(() => {
        this.loading = false;
        this.cdr.detectChanges(); // ✅ force UI update after loading flag changes
      }))
      .subscribe({
        next: (data) => {
          this.rendezvousList = Array.isArray(data) ? [...data] : [];
          this.rebuild();
          this.cdr.detectChanges(); // ✅ force UI update after lists update
        },
        error: () => {
          this.rendezvousList = [];
  ...
  rebuild(): void {
    ...
    if (term) {
      list = list.filter(r => {
        ...
        const medecinName = (r.medecin?.nomMedecin || this.getMedecinName(r.medecin?.id) || '').toLowerCase();
