...
export class MedicalRecordsComponent implements OnInit {
  ...
  private apiUrl = 'http://localhost:8083/api/medical-records';
  ...
  loadRecords(): void {
    ...
    this.http.get<ApiResponse<MedicalRecord[]>>(this.apiUrl).subscribe({
      ...
      error: (err) => {
        ...
        this.error = status === 0 || err?.message?.includes('Unknown Error')
          ? 'Medical service (port 8083) is not running. Run start-all.bat and wait until the "Medical Service (8083)" window shows "Started".'
