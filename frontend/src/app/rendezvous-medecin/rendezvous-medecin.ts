...
import { ToastService } from '../../../public/toast.service';
...
export class RendezvousMedecinComponent implements OnInit {
  ...
  private apiUrl = 'http://localhost:8089/rdv/api/rendezvous';
  private optionsUrl = 'http://localhost:8089/rdv/api/options';
  ...
  terminerRendezVous(rdv: any): void {
    ...
    this.http.put(`${this.apiUrl}/${rdv.id}`, { status: 'TERMINE' }).subscribe({
      next: () => {
